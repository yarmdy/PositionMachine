...
(function ($) {
    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                ...
                */
                ...
                cbiconpath: "/script/wdtree/images",
                ...
                */
                onnodeclick: false,
                /**
                * @description {Config} oncheckcompleted:function()
                * Fired after all related check box state changed after one check box is clicked on.
                */
        ...
        function nodeclick(e) {
            ...
            //alert(et.tagName);
            if (et.tagName == "SPAN") {//如果是选择文字,重新查找图片IMG
                et = et.parentNode.parentNode.childNodes[2];
            }
            if (et.tagName == "IMG") {
                ...
                else if ($(et).hasClass("bbit-tree-node-cb")) // click on checkbox
                {
                    ...
                    if (r != false) {
                        ...
                        if (dfop.oncheckcompleted) {
                            dfop.oncheckcompleted.call();
                        }
                    }
                }
            }
        ...
        function findItem(items, itemid) {
            for (var i = 0, l = items.length; i < l; i++) {
                if (items[i].showcheck && items[i].id == itemid) {
                    return items[i];
                }
                if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
                    var t = findItem(items[i].ChildNodes, itemid);
                    if (t != null) {
                        return t;
                    }
                }
        ...
        function findItems(items, itemid, c) {
            for (var i = 0, l = items.length; i < l; i++) {
                if (items[i].showcheck && items[i].id == itemid) {
                    c.push(items[i]);
                    return;
                ...
                if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
                    findItems(items[i].ChildNodes, itemid, c);
                    if (c.length > 0) {
                        c.push(items[i]);
                        return;
                    }
        ...
        function doCheckNode(items, itemid, docheck) {
            var nid = itemid.replace(/[^\w-]/gi, "_");
            var node = $("#" + id + "_" + nid);
            if (node.length > 0) {
                var path = node.attr("tpath");
                var item = getItem(path);
                check(item, docheck ? 1 : 0, 1);
                bubble(check, item, docheck ? 1 : 0);
            } else {
                ...
                //}
                var c = [];
                findItems(items, itemid, c);
                for (var i = 0, l = c.length; i < l; i++) {
                    if (c[i].hasChildren) {
                        check(c[i], docheck ? 1 : 0, 0);
                    } else {
                        check(c[i], docheck ? 1 : 0, 1);
                    }
                }
            }
        }
        ...
        function getLeafCk(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
                var item = items[i];
                (item.showcheck == true && item.checkstate == 1 && !item.hasChildren) && c.push(fn(item));
                if (item.ChildNodes != null && item.ChildNodes.length > 0) {
                    getLeafCk(item.ChildNodes, c, fn);
                }
            }
        }
        function getLeafCkWithHtml(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
                var item = items[i];
                //(item.showcheck == true && item.checkstate == 1 && !item.hasChildren) && c.push(fn(item));
                if (item.showcheck == true && item.checkstate == 1 && !item.hasChildren) {
                    checkedHtml.push('<li id="L' + item.id + '">' + item.text + '<img onclick="rV(\'' + item.id + '\')" src="/script/wdtree/img/rv.gif"/></li>');
                    item.parent = null;
                    c.push(fn(item));
                    continue;
                }
                if (item.ChildNodes != null && item.ChildNodes.length > 0) {
                    getLeafCkWithHtml(item.ChildNodes, c, fn);
                }
            }
        }
        ...
        me[0].t = {
            ...
            getSelectedLeafNodes: function (gethtml) {
                var s = [];
                checkedHtml = [];
                if (gethtml) {
                    getLeafCkWithHtml(treenodes, s, function (item) { return item; });
                } else {
                    getLeafCk(treenodes, s, function (item) { return item; });
                }
                var result = {
                    Nodes: s,
                    Htmls: checkedHtml
            ...
            checkNode: function (itemid, docheck) {
                doCheckNode(treenodes, itemid, docheck);
            }
        };
        return me;
    };
    ...
    $.fn.getCheckedLeafNodes = function (gethtml) {
        if (this[0].t) {
            return this[0].t.getSelectedLeafNodes(gethtml);
        }
        return null;
    }
    ...
    $.fn.checkNode = function (itemid, docheck) {
        if (this[0].t) {
            return this[0].t.checkNode(itemid, docheck);
        }
    };
})(jQuery);