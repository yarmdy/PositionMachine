...
jBox.prototype.setContent = function(content) {
	...
	switch (jQuery.type(content)) {
	    case 'string':
	        ...
	        {
	            html = "<img style=\"width:150px;height:150px;\" src=\"" + html.split("]")[1] + "\"/>";
	        }
            
	        this.content.html(html);
	        break;
	    case 'object':
	        this.content.children().css({ display: 'none' });
	        this.options.content.appendTo(this.content).css({ display: 'block' });
	        break;
	}
	this.position();
	return this;
